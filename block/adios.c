...
// Since 7.0 the async throttling depth lives in request_queue::async_depth
// and is user-tunable through /sys/block/<dev>/queue/async_depth, which
// invokes this callback. Mirror it instead of re-deriving from nr_requests
// so that the knob actually takes effect (mq-deadline/kyber/bfq do the same).
...
put_eq:
	/*
	 * Do NOT kobject_put(&eq->kobj) here: when ->init_sched() returns an
	 * error, blk_mq_init_sched() drops the last reference itself (see the
	 * "out:" label there). mq-deadline/bfq/kyber all just return the errno.
	 */
	return ret;
}
