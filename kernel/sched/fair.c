...
u64 avg_vruntime(struct cfs_rq *cfs_rq)
{
	...
	if (weight) {
		...
		if (curr) {
			unsigned long w = scale_load_down(curr->h_load.weight);
...
static int vruntime_eligible(struct cfs_rq *cfs_rq, u64 vruntime)
{
	...
	if (curr && curr->on_rq) {
		unsigned long weight = scale_load_down(curr->h_load.weight);
			   ...
			   unsigned long weight, bool on_rq)
{
	...
	if (rel_vprot)
		vprot = div_s64(vprot * se->h_load.weight, weight);
...
place_entity(struct cfs_rq *cfs_rq, struct sched_entity *se, int flags)
{
	...
	if (sched_feat(PLACE_LAG) && nr_queued && se->vlag) {
		...
		if (curr && curr->on_rq)
			load += scale_load_down(curr->h_load.weight);

		lag *= load + scale_load_down(se->h_load.weight);
...
static void wakeup_preempt_fair(struct rq *rq, struct task_struct *p, int wake_flags)
{
	...
	struct task_struct *donor = rq->donor;
	struct sched_entity *se = &donor->se, *pse = &p->se, *nse;
	struct cfs_rq *cfs_rq = &rq->cfs;
